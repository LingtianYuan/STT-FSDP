...
from data_prepare import get_dataloaders_from_index_data, get_adj_dis_matrix
from STTFSDP import STTFSDP
...
def twohop(adj_mx):
    adj_mx = np.dot(adj_mx, adj_mx)
    num_nodes = adj_mx.shape[0]
    twohopmx = np.zeros((num_nodes, num_nodes), dtype = np.float32)
...
if __name__ == "__main__":
    ...
    # -------------------------------- load model -------------------------------- #
    ADJ_MATRIX_PATH = '../data/{}/{}.csv'.format(args.dataset, args.dataset)
    adj_mx, distance_mx = get_adj_dis_matrix(ADJ_MATRIX_PATH, cfg.get("num_nodes"), False,
                                             '../data/{}/{}.txt'.format(args.dataset, args.dataset))
    adj_mx_2hop = twohop(adj_mx)
    num_nodes = adj_mx.shape[0]
    model = STTFSDP(**cfg["model_args"], adj_mx = adj_mx_2hop)
    # ------------------------------- make log file ------------------------------ #
